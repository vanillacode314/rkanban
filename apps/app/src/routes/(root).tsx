...
import { For, JSXElement, Match, Show, Switch, createEffect, onMount, untrack } from 'solid-js';
						...
						{(item) => (
							<Switch>
								<Match when={item.type === 'id/node'}>
									<Button class="flex items-center gap-2" as="div" variant="secondary">
										<span
											class={cn(
										...
										/>
										<span>{(item.meta as { node: TNode; path: string }).path}</span>
